...
## 내가 해낸것들 (장바구니API ,주문배송API 구현)

-  장바구니 API
-   상품을 장바구니에 담고 고객의 요청에 따라 장바구니 안에 담긴 상품을 지우고 그것을 이용하여 주문페이지로 넘겨주는 API이다.
-   배운점 : HTTP 메소드인 GET, POST, DELETE, PATCH를 알게 되었습니다.

-  주문배송 API
-   장바구니에 담은 상품들을 주문할때 실제 상세 페이지처럼 보내는사람과 받는사람을 DB에 기록되게 구현하였다.
-   배운점 : POST GET 과 Django 내부모듈인 ENUM을 사용 하였다.
   
## 팀으로써 엄청 배운점

-  문서의 중요성
-  맨 처음엔 프론트엔드와 백엔드 둘 다 말로만 시작하였는데 역시 기록되지 않으면 정말 어렵다는걸 깨달았다. 나중에는 키값이 하나 맞지도 않았고
-  예슬님의 필두로 기록을 하기 시작하여 나중에는 정말 편리함과 중요성을 깨달았다. (이건 꼭 다른 조에게 추천하고싶다!)
